...
class BackupController extends BaseApiController
{
    ...
    public function index(): ResponseInterface
    {
        ...
        $model = model(BackupModel::class);
        $limit  = (int) ($this->request->getGet('limit') ?? 100);
        $offset = (int) ($this->request->getGet('offset') ?? 0);
        $limit  = max(1, min($limit, 500));
        $offset = max(0, $offset);

        return $this->ok($model->findAllSafe($limit, $offset), 'Backup list');
    }
    ...
    public function show($id = null): ResponseInterface
    {
        ...
        if ($row === null) {
            return $this->failNotFoundMessage('Backup not found');
        }

        return $this->ok($row, 'Backup detail');
    }
    ...
    public function create(): ResponseInterface
    {
        ...
        return $this->ok($model->findSafe($id), 'Backup created', 201);
    }
    ...
    public function update($id = null): ResponseInterface
    {
        ...
        return $this->ok($model->findSafe($id), 'Backup updated');
    }
